...
def test_add_bar_route(client: FlaskClient):
    ...
    with patch("bar_recs.bar_recs.add_bar") as mock_add_bar:
        mock_add_bar.return_value = MagicMock(
            inserted_id=ObjectId("123456789012345678901234")
        )
...
def test_recommend_route(client: FlaskClient):
    ...
    with patch(
        "bar_recs.bar_recs.recommend_bars_for_user", return_value=mock_recommendations
    ):
...
def test_recommend_bars_for_user():
    ...
    with patch("bar_recs.bar_recs.get_all_bars", return_value=mock_bars), patch(
        "bar_recs.bar_recs.db.ratings.find", return_value=mock_ratings
...
def test_train_recommender_system():
    ...
    with patch("bar_recs.bar_recs.load_ratings_data", return_value=mock_dataset), patch(
        "bar_recs.bar_recs.train_test_split", return_value=(MagicMock(), MagicMock())
