...
class Post:
    ...
    def validate_title(title):
        if not title or len(title.strip()) < Post.TITLE_MIN_LENGTH:
            raise ValueError(f"Title must be at least {Post.TITLE_MIN_LENGTH} characters long")
        if len(title) > Post.TITLE_MAX_LENGTH:
            raise ValueError(f"Title must not exceed {Post.TITLE_MAX_LENGTH} characters")

    @staticmethod
    def validate_content(content):
        if not content or len(content.strip()) < Post.CONTENT_MIN_LENGTH:
            raise ValueError(f"Content must be at least {Post.CONTENT_MIN_LENGTH} characters long")
        if len(content) > Post.CONTENT_MAX_LENGTH:
            raise ValueError(f"Content must not exceed {Post.CONTENT_MAX_LENGTH} characters")
    ...
    def validate_tags(tags):
        ...
        for tag in tags:
            ...
            if len(tag) > Post.TAG_MAX_LENGTH:
                raise ValueError(f"Tag length must not exceed {Post.TAG_MAX_LENGTH} characters")
