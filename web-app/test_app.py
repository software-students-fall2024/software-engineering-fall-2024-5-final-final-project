...
def test_ai_ml_route(client):
    ...
    assert response.status_code == 200
    assert (
        b"ai_machine_learning.html" not in response.data
    )  # Assuming template rendering
...
def test_play_against_ai_success(client):
    payload = {"player_name": "TestPlayer", "choice": "rock"}
...
def test_play_against_ai_invalid_choice(client):
    payload = {"player_name": "TestPlayer", "choice": "invalid_choice"}
    response = client.post("/play/ai", json=payload)
    assert response.status_code == 400
    json_data = response.get_json()
...
def test_socketio_random_match_waiting(sio_client):
    with patch("app.waiting_players", []), patch("app.active_games", {}), patch(
        "app.socketio.start_background_task"
    ) as mock_start_bg_task:
...
def test_socketio_random_match_match_found(sio_client):
    with patch(
        "app.waiting_players",
        [{"player_id": "id1", "player_name": "Player1", "sid": "sid1"}],
    ), patch("app.active_games", {}) as mock_active_games, patch(
        "app.socketio.start_background_task"
    ) as mock_start_bg_task:
...
def test_socketio_cancel_waiting(sio_client):
    with patch(
        ...
        assert (
            received[0]["args"][0]["message"] == "You canceled waiting for an opponent."
        )
...
def test_socketio_join_game(sio_client):
    with patch(
    ...
    ), patch("app.join_room") as mock_join_room, patch(
        "app.socketio.emit"
    ) as mock_emit:
        ...
        mock_emit.assert_called_with(
            "start_game", {"message": "Game starting soon!"}, room="game123"
        )
...
def test_socketio_submit_choice(sio_client):
    with patch(
        ...
        sio_client.emit(
            "submit_choice",
            {"game_id": "game123", "player_id": "id1", "choice": "rock"},
        )
        received = sio_client.get_received()
        assert len(received) == 0  # No immediate response
        ...
        sio_client.emit(
            "submit_choice",
            {"game_id": "game123", "player_id": "id2", "choice": "scissors"},
        )
        received = sio_client.get_received()
...
def test_socketio_submit_choice_invalid_player(sio_client):
    with patch(
        ...
        sio_client.emit(
            "submit_choice",
            {"game_id": "game123", "player_id": "invalid_id", "choice": "rock"},
        )
        received = sio_client.get_received()
